...
for i in range(10):
	...
	autoencoder.fit([X_train,X_train], [X_train,X_train], nb_epoch=1, batch_size=128, callbacks=[siamese])

	print "Evaluating RNN Autoencoder <Seq2Seq> on training set"
	print autoencoder.evaluate([X_train,X_train], [X_train,X_train], batch_size=128)
