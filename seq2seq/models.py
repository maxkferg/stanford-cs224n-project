...
def Seq2Seq(output_dim, output_length, hidden_dim=None, depth=1, broadcast_state=True, inner_broadcast_state=True, teacher_force=False, peek=False, dropout=0., **kwargs):
	...
	for i in range(2):
		...
		# Collect the input and output of the RNN
		modelInputs.append(input)
		modelOutputs.append(decoded_seq)
		modelMidpoints.append(encoded_seq)

	model = Model(input=modelInputs, output=modelOutputs)
	model.midpoints = modelMidpoints
	return model
